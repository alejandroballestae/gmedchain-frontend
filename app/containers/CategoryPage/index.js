...
async function getProducts() {
  ...
  const response = await http.get('/product').then(data => data)
    .catch(error => {
      console.error(error);
  });
  return response;
...
async function addProduct(product, history){
  ...
  const response = await http.put('/cart',product).then(data => data)
    .catch(error => {
      if (error.response) {
...
class CategoryPage extends Component {
  ...
  addProductToCartHandler = (event, id) => {
    event.preventDefault();
      const serverProduct = this.state.products.find((product) => product._id === id);
  ...
  removeProductToCartHandler = id => {
      const cartProduct = this.state.cartLineState.find((product) => product.id === id);
      if(cartProduct.quantity === 1) {
        this.setState({cartLineState:this.state.cartLineState.filter((item) => item.id !== id)});
      } else {
    ...
    console.log('-----------------logprops');
      ...
      if(this.props.location.state == undefined){
        ...
        if(this.props.location.state.searchText){
          let url = "/search-product/"+this.props.location.state.searchText
